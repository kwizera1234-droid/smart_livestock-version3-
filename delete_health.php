...
$check = $conn->prepare('SELECT id FROM health_records WHERE id = ? LIMIT 1');
$check->bind_param('i', $id);
$check->execute();
if ($check->get_result()->num_rows === 0) {
    ob_clean(); echo json_encode(['success' => false, 'message' => 'Health record not found (id=' . $id . ')']); exit();
}
$check->close();
...
if ($stmt->execute()) {
    ob_clean(); echo json_encode(['success' => true, 'message' => 'Health record deleted', 'id' => $id]);
} else {
